...
def generate_ai_content(sys_prompt, user_prompt):
    ...
    try:
        # USE THE DETECTED MODEL (Do not hardcode 'gemini-1.5-flash')
        # ACTIVE_MODEL_NAME is determined at the top of your script
        ...
        print(f"❌ AI Error: {e}")
        print("⚠️ SWITCHING TO DEMO MODE (Returning fallback response)")
        
        # --- DEMO MODE FAIL-SAFE ---
        # If the API fails (404, 429 Quota, No Internet), return this valid JSON.
        # This ensures your UI ALWAYS works during the presentation.
        return """
        {
            "disease": "Viral Fever (Demo Diagnosis)",
            "description": "The AI connection encountered a limit, so this is a simulated response. The patient shows classic signs of viral infection including elevated temperature and fatigue.",
            "medicine": "• **Paracetamol (Dolo 650)**: 1 tablet every 6 hours after food.\\n• **Vitamin C**: 1 tablet daily for immunity.",
            "precautions": "• Stay hydrated by drinking 3L of water.\\n• Isolate if coughing persists.",
            "workouts": "• Light walking only.\\n• Avoid heavy lifting.",
            "diet": "• Eat: **Khichdi**, **Soup**, **Boiled Eggs**\\n• Avoid: Cold water, oily food"
        }
        """
...
# 3. GEMINI CALL
    if API_KEY:
        sys_prompt = f"You are a medical AI. User speaks {lang_name}. Return strictly valid JSON."
        
        # COMBINED PROMPT: Translation + Shopping Links
        user_prompt = f"""
        Patient Profile: {age} years old, {gender}.
        Medical History: {history_text}.
        Current Symptoms: {symptoms}.
        Vitals Analysis: {vital_text}.
        Target Language: {lang_name}.

        Task: Act as a doctor. Provide a structured care plan.
        
        CRITICAL INSTRUCTIONS:
        1. **Translate ALL content** (Description, Advice, Dosage instructions) into {lang_name}.
        2. Keep the **JSON Keys** (disease, medicine, diet, etc.) in ENGLISH.
        3. **SHOPPING FORMATTING RULES**:
           - You MUST wrap specific Medicine names and Food ingredients in **double asterisks**.
           - Keep the **Bolded Keyword** in ENGLISH (or English + Local) so the shopping link works correctly.
           - Example: "Take **Paracetamol** 500mg" (Rest of sentence in {lang_name}).
           - Example: "Eat **Spinach** (ಪಾಲಕ್)" (Rest of sentence in {lang_name}).
        
        RETURN JSON FORMAT:
        {{
            "disease": "Condition Name in {lang_name}",
            "description": "Brief explanation in {lang_name}",
            "medicine": "• **Medicine Name**: Dosage instructions in {lang_name}\\n• **Supplement Name**: Instructions in {lang_name}",
            "precautions": "• Precaution 1 in {lang_name}\\n• Precaution 2 in {lang_name}",
            "workouts": "• Exercise 1 in {lang_name}\\n• Exercise 2 in {lang_name}",
            "diet": "• Eat: **Food Item**, **Food Item** (in {lang_name})\\n• Avoid: Food Item"
        }}
        """
...
def chat():
    ...
    if image_data:
        ...
        Return valid JSON. Values must be single markdown strings.
    ...
    # 3. TEXT CHAT MODE (Detailed & Structured)
    else:
        # ✅ UPDATED SYSTEM INSTRUCTION
        sys_instruction = f"""
        You are a Senior Medical Consultant AI. Language: {lang}. 
        Your goal is to provide structured, easy-to-read, and comprehensive medical advice.
        
        GUIDELINES:
        1. **Format**: Use **Bold** for headers (e.g. **Diagnosis**) and bullet points (•) for lists.
        2. **Diagrams**: Assess if the user would understand better with a diagram. If yes, insert a tag
 where X is a specific, domain-specific query (e.g., 

[Image of human digestive system]
, ). Avoid generic images like 'person in pain'. Place the tag at the start of the relevant section.
        3. **Content**: Explain the 'Why' and 'How'. Be specific with dosage and preparation.
        """
        ...
        [INSTRUCTIONS]
        1. Analyze the symptom deepy.
        2. Suggest 2-3 standard medicines with dosage.
        3. Suggest 2 herbal remedies with preparation steps.
        4. Return VALID JSON.
        
        [REQUIRED JSON FORMAT]
        {{
            "medical": " (Optional)\\n\\n**Likely Cause**\\n• Explanation of cause.\\n\\n**Recommended Medicines**\\n• **Paracetamol**: 500mg after food.\\n• **Ibuprofen**: 400mg for pain.\\n\\n**Precautions**\\n• Avoid empty stomach.",
        ...
        if ai_text:
            try:
                ...
                response["medical"] = ai_text.replace('"', "'") 
...
def doctors():
    d = request.json
    spec = d.get('specialty', 'Hospital')
    lat = d.get('lat')
    lon = d.get('lon')
    loc_name = d.get('location', 'Unknown Location')
    
    print(f"🔎 Searching for {spec} near {lat}, {lon}...")
    
    results = []

    # --- STRATEGY 1: REAL DATA (Overpass API) ---
    if lat and lon:
        try:
            # We use a 'Union' query which is faster than Regex
            ...
            if r.status_code == 200:
                ...
                print(f"✅ OSM found {len(elements)} raw results.")
                
                for el in elements[:10]: # Limit to 10
                    name = el.get('tags', {}).get('name')
                    # Only add if it has a name (cleaner list)
                    if name:
                        results.append({
                            "name": name, 
    ...
    # --- STRATEGY 2: AI FALLBACK (If OSM returns 0 results) ---
    if not results:
        print("⚠️ No OSM data found. Trying AI...")
        sys_prompt = "You are a location data generator. Return strictly valid JSON."
        user_prompt = f"""
        Generate 4 realistic {spec} names and addresses in {loc_name}. 
        They don't need to be real coordinates, just realistic names.
        Format: [ {{"name": "X", "address": "Y"}} ]
        """
        ...
        if ai_text:
            try:
                ...
                # Assign fake coordinates near the user so they show on the map
                for i, item in enumerate(ai_results):
                    # Add small random offset to user's lat/lon
                    offset_lat = (random.random() - 0.5) * 0.02 
                    offset_lon = (random.random() - 0.5) * 0.02
                    
                    results.append({
                        "name": item.get('name', f"{spec} Center"),
                        "address": item.get('address', loc_name),
                        "rating": "4.2 ★",
                        "lat": float(lat) + offset_lat if lat else None,
                        "lon": float(lon) + offset_lon if lon else None
                    })
            except Exception as e:
                print(f"❌ AI Parse Error: {e}")

    # --- STRATEGY 3: HARDCODED DEMO (Last Resort) ---
    # If BOTH OpenStreetMap and AI fail, generate dummy data so the UI isn't empty.
    if not results and lat and lon:
        print("⚠️ All lookups failed. Generating DEMO pins.")
        demos = [
            ("City Care Hospital", 0.002, 0.002),
            ("Green Cross Pharmacy", -0.002, 0.001),
            ("Dr. Smith's Clinic", 0.001, -0.003),
            ("Sunrise Health Center", -0.003, -0.002)
        ]
        for name, off_lat, off_lon in demos:
            results.append({
                "name": name + " (Demo)",
                "address": "Nearby Location",
                "rating": "4.0 ★",
                "lat": float(lat) + off_lat,
                "lon": float(lon) + off_lon
            })

    # Return center so map knows where to focus even if results came from AI
    return jsonify({
        "result": results, 
